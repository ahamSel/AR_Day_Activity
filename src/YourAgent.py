...
class YourAgent(DriveInterface):
    ...
    def get_next_move(self, sensor_data: dict) -> DriveMove:
        ...
        if len(self.path) == 0:
            if self.need_to_find_target_pod:
                ...
                raise Exception('Advanced mode solver not implemented yet for YourAgent')
            else:
    ...
    def dfs_solve_path_to_goal(self, sensor_data: dict, goal: list[int]):
        ...
        while len(paths) > 0:
            current_path = paths.pop(len(paths)-2)
            ...
            for state in self.list_all_next_possible_states(curr_state):
                if state not in visited_states and self.is_state_in_bounds(state, sensor_data):
                    if self.will_next_state_collide(state, sensor_data):
                        continue
    ...
    def will_next_state_collide(self, state: DriveState, sensor_data: dict) -> bool:
        ...
        for drive_state in sensor_data[SensorData.DRIVE_LOCATIONS]:
            if state.x == drive_state[0] and state.y == drive_state[1]:
                return True
